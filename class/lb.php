...
class LeftBack
{
    ...
    function tampil($cari)
    {
        ...
        $query = $kon->query("SELECT * FROM pm_lb, pm_pemain where pm_lb.kdpemain=pm_pemain.kdpemain and pm_pemain.namapemain like '%$cari%'");
        ...
        while ($row = $query->fetch_array()) {
            ...
            echo '
						<tr>
							...
							<td style="text-align: center"><a href="editnilai_gk.php?kdnilai2=' . $row['kdnilai2'] . '">
    ...
    function input(
        ...
        $query = "INSERT INTO pm_lb (kdpemain, nilai_pc, target_pc, selisih_pc, nilai_bobot_pc,
						 ...
						 VALUES ('$kdpemain','$nilai_pc','$target_pc','$selisih_pc','$nilai_bobot_pc',
                                                                  ...
                                                                  '$nilai_cr','$target_cr','$selisih_cr','$nilai_bobot_cr',
																	'$nilai_cf_A2','$nilai_sf_A2','$nilai_tot_A2')";
        $qcek = $kon->query("select * from pm_lb where kdnilai2='$kdnilai2'");
        $jmbaris = $qcek->num_rows;
        if ($jmbaris == 1) {
            echo "<script>alert('Data Sudah Ada'); window.location='inputnilai_lb.php';</script>";
        } else {
            $simpan = $kon->query("$query");

            if ($simpan) {
                echo "<script>alert('Data Berhasil Ditambahkan'); window.location='inputnilai_lb.php';</script>";
            } else {
                echo "<script>alert('Gagal Menambahkan Data'); window.location='inputnilai_lb.php';</script>";
            }
        }
    }
    ...
    function update(
        ...
        $query = "UPDATE pm_lb SET kdpemain='$kdpemain',
    ...
    function factor($cari)
    {
        ...
        $query = $kon->query("SELECT * FROM pm_lb, pm_pemain WHERE pm_lb.kdpemain=pm_pemain.kdpemain AND pm_pemain.namapemain like '%$cari%' ORDER BY nilai_tot_A2 DESC");
        ...
        $qkriteria_core = $kon->query("select distinct jenis from pm_kriteria where kdkriteria = 'LBTM' or kdkriteria = 'LBDW' or kdkriteria = 'LBCA'");
        $dtcore = $qkriteria_core->fetch_array();
        $jenis_core = $dtcore['jenis'];

        $qkriteria_sec = $kon->query("select distinct jenis from pm_kriteria where kdkriteria = 'LBPC' or kdkriteria = 'LBCR'");
