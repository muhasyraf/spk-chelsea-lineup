...
class RightMidfielder
{
    ...
    function tampil($cari)
    {
        ...
        $query = $kon->query("SELECT * FROM pm_rmf, pm_pemain where pm_rmf.kdpemain=pm_pemain.kdpemain and pm_pemain.namapemain like '%$cari%'");
        ...
        while ($row = $query->fetch_array()) {
            ...
            echo '
						<tr>
							...
							<td style="text-align: center"><a href="editnilai_gk.php?kdnilai7=' . $row['kdnilai7'] . '">
    ...
    function input(
        ...
        $query = "INSERT INTO pm_rmf (kdpemain, nilai_pc, target_pc, selisih_pc, nilai_bobot_pc,
						 ...
						 VALUES ('$kdpemain','$nilai_pc','$target_pc','$selisih_pc','$nilai_bobot_pc',
                                                                  ...
                                                                  '$nilai_ca','$target_ca','$selisih_ca','$nilai_bobot_ca',
																	'$nilai_cf_A7','$nilai_sf_A7','$nilai_tot_A7')";
        $qcek = $kon->query("select * from pm_rmf where kdnilai7='$kdnilai7'");
        $jmbaris = $qcek->num_rows;
        if ($jmbaris == 1) {
            echo "<script>alert('Data Sudah Ada'); window.location='inputnilai_rmf.php';</script>";
        } else {
            $simpan = $kon->query("$query");

            if ($simpan) {
                echo "<script>alert('Data Berhasil Ditambahkan'); window.location='inputnilai_rmf.php';</script>";
            } else {
                echo "<script>alert('Gagal Menambahkan Data'); window.location='inputnilai_rmf.php';</script>";
            }
        }
    }
    ...
    function update(
        ...
        $query = "UPDATE pm_rmf SET kdpemain='$kdpemain',
							...
							nilai_cf_A7='$nilai_cf_A7', nilai_sf_A7='$nilai_sf_A7', nilai_tot_A7='$nilai_tot_A7'
							WHERE kdnilai7 = '$kdnilai7'";
        $update = $kon->query("$query");

        if ($update) {
            echo "<script>alert('Data Berhasil Diperbarui'); window.location='hasil_rmf.php';</script>";
        } else {
            echo "<script>alert('Gagal Memperbarui Data'); window.location='hasil_rmf.php';</script>";
        }
    }
    ...
    function factor($cari)
    {
        ...
        $query = $kon->query("SELECT * FROM pm_rmf, pm_pemain WHERE pm_rmf.kdpemain=pm_pemain.kdpemain AND pm_pemain.namapemain like '%$cari%' ORDER BY nilai_tot_A7 DESC");
        ...
        while ($row = $query->fetch_array()) {
            ...
            echo '
						<tr>
							...
							<td style="text-align: center">' . round($row['nilai_cf_A7'], 2) . '</td>
							<td style="text-align: center">' . round($row['nilai_sf_A7'], 2) . '</td>
							<td style="text-align: center">' . round($row['nilai_tot_A7'], 2) . '</td>
						</tr>';
            $no++;
        }
        $qkriteria_core = $kon->query("select distinct jenis from pm_kriteria where kdkriteria = 'RMPC' or kdkriteria = 'RMAS' or kdkriteria = 'RMCC'");
        $dtcore = $qkriteria_core->fetch_array();
        $jenis_core = $dtcore['jenis'];

        $qkriteria_sec = $kon->query("select distinct jenis from pm_kriteria where kdkriteria = 'RMDW' or kdkriteria = 'RMCA'");
